...
done < <(nix-env --list-generations --profile /nix/var/nix/profiles/system)
...
if [[ "$input" == "all" ]]; then
	...
	keep_count=5
	gens=($(nix-env --list-generations --profile /nix/var/nix/profiles/system | awk '{print $1}' | grep '^[0-9]*$' | sort -n))
	total=${#gens[@]}
...
failed=0
for gen in "${generations_to_delete[@]}"; do
	if sudo nix-env --delete-generations --profile /nix/var/nix/profiles/system "$gen" 2>/dev/null; then
